...
function Catservices(props) {
  // const [catService, setcatService] = useState(props);

  return (
    <>
   
      <Link to={`/service/${props.selectedCat.name}/${props.selectedCat.id}`} key={props.selectedCat.cate} data-id="" className="service-container col-lg-12">
          ...
          <div className="serv-name">
            ...
            <div className="serv-price">
              {props.selectedCat.price}
              <span>
                {" "}
                / <i className="far fa-clock"></i> {props.selectedCat.time}
              </span>
            </div>
          </div>
          </div>
          <p className="service-descrption">{props.selectedCat.description}</p>
      </Link>
